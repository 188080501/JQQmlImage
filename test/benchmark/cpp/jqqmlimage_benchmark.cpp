...
void JQQmlImageBenchmark::cleanupTestCase()
{
    ...
    for ( const auto &decodeResult: decodeTimeResults_ )
    {
        qDebug() << "Decode result(time):" << QString( "%1.%2%" ).arg( static_cast< int >( decodeResult * 100 ) ).arg( static_cast< int >( decodeResult * 1000 ) % 10 );
    }

    for ( const auto &decodeResult: decodeSizeResults_ )
    {
        qDebug() << "Decode result(size):" << QString( "%1.%2%" ).arg( static_cast< int >( decodeResult * 100 ) ).arg( static_cast< int >( decodeResult * 1000 ) % 10 );
    }
}
...
//    }
    ...
    decodeTimeResults_.insert( decodeTimeResult );
}
